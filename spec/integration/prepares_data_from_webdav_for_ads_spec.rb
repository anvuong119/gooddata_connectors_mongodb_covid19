...
describe 'Downloading data from WebDav and preparing metadata, batch and files for ads integrator', type: :feature do
  before :all do
    ...
    WebDavHelper = GoodData::Connectors::DownloaderCsv::WebDavHelper
    CONFIGURATION_OPTIONS = {
        '$WEBDAV_HOST' => GoodData::Connectors::DownloaderCsv::ConnectionHelper::WEBDAV_HOST,
        '$WEBDAV_USERNAME' => GoodData::Connectors::DownloaderCsv::ConnectionHelper::WEBDAV_USERNAME,
        '$WEBDAV_PASSWORD' => GoodData::Connectors::DownloaderCsv::ConnectionHelper::WEBDAV_PASSWORD
    }
  ...
  it 'prepares data from uncompressed file without manifest and feed files' do
    ...
    S3Helper.upload_file('spec/data/configurations/webdav/configuration_1.json', remote_config_path, replacement)

    # prepare metadata on webdav
    WebDavHelper.upload('spec/data/files/feature/events.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/events.csv")
    ...
    data = File.open(Dir["tmp/*_data_#{@padding}.csv"].first).read
  ...
  it 'prepares data from compressed file with manifests and feed files (x2)' do
    ...
    remote_config_path = S3Helper.generate_remote_path('configuration.json', remote_testing_folder)
    S3Helper.upload_file('spec/data/configurations/webdav/configuration_2.json', remote_config_path, replacement)

    # prepare data
    WebDavHelper.upload('spec/data/files/feature/events0.gz', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/events0.gz")

    # prepare manifest
    WebDavHelper.upload('spec/data/manifests/webdav/manifest_1.20160714105300.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/manifests/manifest_1.20160714105300.csv", replacement)

    # prepare feed
    WebDavHelper.upload('spec/data/files/feature/feed', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/feed")
    ...
    # prepare next data
    WebDavHelper.upload('spec/data/files/feature/events.gz', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/events.gz")

    # prepare next manifest
    WebDavHelper.upload('spec/data/manifests/webdav/manifest_2.20160914105300.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/manifests/manifest_2.20160914105300.csv", replacement)

    # prepare feed
    WebDavHelper.upload('spec/data/files/feature/feed4', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/feed")
  ...
  it 'prepares data from uncompressed file with manifests and feed files, three runs, first full, other two inc' do
    ...
    # prepare configuration
    remote_config_path = S3Helper.generate_remote_path('configuration.json', remote_testing_folder)
    S3Helper.upload_file('spec/data/configurations/webdav/configuration_5.json', remote_config_path, replacement)
    # prepare 3 data files
    WebDavHelper.upload('spec/data/files/feature/events0.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/events0.csv")
    WebDavHelper.upload('spec/data/files/feature/events.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/events.csv")
    WebDavHelper.upload('spec/data/files/feature/events1.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/events1.csv")
    # prepare 3 manifests
    WebDavHelper.upload('spec/data/manifests/webdav/manifest_1.20160714105302.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/manifests/manifest_1.20160714105302.csv", replacement)
    WebDavHelper.upload('spec/data/manifests/webdav/manifest_2.20160714105302.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/manifests/manifest_2.20160714105302.csv", replacement)
    WebDavHelper.upload('spec/data/manifests/webdav/manifest_3.20160714105302.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/manifests/manifest_3.20160714105302.csv", replacement)
    # prepare feed
    WebDavHelper.upload('spec/data/files/feature/feed', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/feed")
  ...
  it 'prepares data from xlsx file without manifests with feed file (City full)' do
    ...
    # prepare configuration
    remote_config_path = S3Helper.generate_remote_path('configuration.json', remote_testing_folder)
    S3Helper.upload_file('spec/data/configurations/webdav/configuration_3.json', remote_config_path, replacement)
    # prepare data excel
    WebDavHelper.upload('spec/data/files/data_folder/entities.xlsx', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/entities.xlsx")
    # prepare feed
    WebDavHelper.upload('spec/data/files/feature/feed2', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/feed")
  ...
  it 'prepares data from uncompressed file without manifest and feed files with entity_name parsing' do
    ...
    # prepare configuration
    remote_config_path = S3Helper.generate_remote_path('configuration.json', remote_testing_folder)
    S3Helper.upload_file('spec/data/configurations/webdav/configuration_6.json', remote_config_path, replacement)
    # prepare data
    WebDavHelper.upload('spec/data/files/feature/events.csv', "#{ROOT_TEST_FOLDER}/integration_test/#{remote_testing_folder}/Event_20160914_R.pd.csv")
